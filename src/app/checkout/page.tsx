...
function CheckoutContent() {
  ...
  const paymentMethods = [
    { id: "cod" as const, label: "Cash on Delivery", number: null, bg: "#C79524" },
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      saveIncompleteOrder(
        ...
        .then((order) => {
          if (order?.Id) {
            setIncompleteOrderId(order.Id);
            if (leadTrackedOrderIdRef.current !== order.Id) {
              leadTrackedOrderIdRef.current = order.Id;
              trackPixelEvent(
                "Lead",
                {
                  content_ids: items.map((i) => i.id),
                  content_name: "Incomplete Checkout",
                  content_type: "product",
                  value: grandTotal,
                  currency: "BDT",
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      trackPixelEvent(
        "AddPaymentInfo",
        { content_ids: items.map((i) => i.id), content_name: "Order Pending Reconfirm",
      ...
      clearCart();
      router.push(
        `/re-confirm?orderId=${encodeURIComponent(String(order.Id))}&invoiceId=${encodeURIComponent(order.invoiceId || order.orderId || "")}&phone=${encodeURIComponent(normalizedPhone)}`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : "";
        ...
        <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 16px" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", textAlign: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.10)", maxWidth: 460, width: "100%" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Link href="/track-order" style={{ display: "block", background: "#C79524", color: "#fff", padding: "13px 0", borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
                Order Track করুন
              </Link>
              ...
              <div style={{ padding: "30px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  <label style={{ fontSize: 14, fontWeight: 600, color: "#333", display: "block", marginBottom: 8 }}>
                    আপনার নাম <span style={{ color: "#D7262E" }}>*</span>
                  </label>
                  <div style={fieldWrap}>
                    <span style={iconBox}>
                ...
                <div>
                  <label style={{ fontSize: 14, fontWeight: 600, color: "#333", display: "block", marginBottom: 8 }}>
                    ফোন নাম্বার <span style={{ color: "#D7262E" }}>*</span>
                  </label>
                  <div style={fieldWrap}>
                    <span style={iconBox}>
                      <svg width="18" height="18" fill="none" stroke="#888" strokeWidth="1.8" viewBox="0 0 24 24">
                ...
                <div>
                  <label style={{ fontSize: 14, fontWeight: 600, color: "#333", display: "block", marginBottom: 8 }}>
                    আপনার পুরো ঠিকানা <span style={{ color: "#D7262E" }}>*</span>
                  </label>
                  <div style={fieldWrap}>
                    <span style={iconBox}>
                      <svg width="18" height="18" fill="none" stroke="#888" strokeWidth="1.8" viewBox="0 0 24 24">
                ...
                <div>
                  <label style={{ fontSize: 14, fontWeight: 600, color: "#333", display: "block", marginBottom: 8 }}>
                    জেলা নির্বাচন করুন <span style={{ color: "#D7262E" }}>*</span>
                  </label>
                  ...
                  {district && (
                    <p style={{ margin: "6px 0 0", fontSize: 13, color: "#C79524", fontWeight: 500 }}>
                ...
                <div>
                  <label style={{ fontSize: 14, fontWeight: 600, color: "#333", display: "block", marginBottom: 12 }}>
                    পেমেন্ট মেথড <span style={{ color: "#D7262E" }}>*</span>
                  </label>
                ...
                {errorMsg && (
                  <p style={{ color: "#D7262E", fontSize: 13, margin: 0, background: "#fff1f1", padding: "10px 14px", borderRadius: 6, border: "1px solid #fca5a5" }}>
                    {errorMsg}
                  </p>
                )}
                  ...
                  items.map((item, idx) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          width: 24, height: 24, borderRadius: "50%", background: "#D7262E",
              ...
              <div style={{ padding: "0 22px 22px" }}>
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button
                    ...
                    style={{
                      background: couponLoading ? "#9ca3af" : "#C79524", color: "#fff", border: "none", borderRadius: 8,
